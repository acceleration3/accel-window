...
//        if(/* major version number */LOBYTE(LOWORD(GetVersion())) >= 6)
    ...
    if(IsZoomed(sizemove->hwnd))
    {
        ...
        if (sizemove->ptCapture.x < (w / 2))
        {
            place.rcNormalPosition.left = 0;
            place.rcNormalPosition.right = w;
        }
        else if(sizemove->ptCapture.x > (minfo.rcWork.right - (w / 2)))
        {
            place.rcNormalPosition.right = minfo.rcWork.right;
            place.rcNormalPosition.left = place.rcNormalPosition.right - w;
        }
        else
        {
            int half = w / 2;
            place.rcNormalPosition.left = sizemove->ptCapture.x - half;
            place.rcNormalPosition.right = sizemove->ptCapture.x + (w - half);
        }
    ...
    else if(sizemove->snap.isSnapped)
    {
        ...
        sizemove->rcWin.right = sizemove->rcWin.left + sizemove->snap.rcRestore.right;
...
void SnapFinalize(SIZEMOVEDATA *sizemove)
{
       ...
       && sizemove->snap.rcWork.top != sizemove->snap.rcWork.bottom)
    {
        ...
        else
        {
            ...
            sizemove->snap.rcRestore.bottom = RECTHEIGHT(sizemove->rcWin);
